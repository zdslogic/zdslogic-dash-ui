...
export class ErrorLogsService {
	...
	findLogs(
		...
		const apiUrl = this.createCompleteRoute('errorlog', environment.apiUrl);
	...
	findLogsWithSortAndFilter(
		...
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute('errorlog', environment.apiUrl);
		const paramsx: any = { page: pageNumber, size: pageSize };
		if (sort != null) {
			paramsx.sort = sort.property + ',' + sort.direction;
		}
		...
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('errorlog/search', environment.apiUrl);

			search = 'recieveTime==' + '\'*' + filter + '*\''
				+ ' or ' + 'errorMessage==' + '\'*' + filter + '*\''
				+ ' or ' + 'client==' + '\'*' + filter + '*\''
				+ ' or ' + 'server==' + '\'*' + filter + '*\'';
