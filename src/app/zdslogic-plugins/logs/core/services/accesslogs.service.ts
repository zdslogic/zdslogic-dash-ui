...
export class AccessLogsService {
	...
	findLogs(
		...
		const apiUrl = this.createCompleteRoute('log', environment.apiUrl);
	...
	findLogsWithSortAndFilter(
		...
		filter = '', sort: PaginationPropertySort,
		pageNumber = 0, pageSize = 3): Observable<any> {
		let apiUrl = this.createCompleteRoute('log', environment.apiUrl);
		const paramsx: any = { page: pageNumber, size: pageSize };
		if (sort != null) {
			paramsx.sort = sort.property + ',' + sort.direction;
		}
		...
		let search: string;
		if (filter !== '') {
			apiUrl = this.createCompleteRoute('log/search', environment.apiUrl);
